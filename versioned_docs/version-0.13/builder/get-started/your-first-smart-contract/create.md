...
The `#[component]` attribute marks this as a Miden [Account component](/reference/miden-base/account). The `count_map` field is a `StorageMap` stored in a named storage slot of the account. In v0.13, storage slots are identified by name rather than explicit index numbers — the slot name is derived automatically from the component's package name and field name (e.g., `miden::component::miden_counter_account::count_map`).
...
Learn more about [note scripts in the Miden documentation](/reference/miden-base/note/).
