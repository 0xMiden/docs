...
The network doesn't store the full account state. Instead, it stores cryptographic commitments — hashes of the code, storage, and vault (see [account design](/reference/miden-base/account/)). Only the account owner (or a public account's observers) sees the actual data.
